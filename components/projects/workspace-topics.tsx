...
export function TopicsPage({ topics, adding, onAdd, onCancel, onOpen }: { topics: Topic[]; adding: boolean; onAdd: (topic: Topic) => void; onCancel: () => void; onOpen: (id: string) => void }) {
  return <>
    {adding ? <form className="mb-8 grid max-w-xl gap-5 border-b border-border pb-8" action={(data) => { const title = String(data.get("title") ?? "").trim(); const question = String(data.get("question") ?? "").trim(); if (!title || !question) return; onAdd({ id: createWorkspaceId(), title, question, cadence: data.get("cadence") === "daily" ? "daily" : "one-time", outline: "", runs: [] }); }}>
