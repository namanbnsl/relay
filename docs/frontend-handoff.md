...
3. Point development at the reported Phase 1 Convex deployment, including `mcpActivity`. Provider execution still requires `EXA_API_KEY` and `RESEARCH_ENABLED=true`; effort defaults to medium. Production promotion remains a separate reviewed action. Do not show the new onboarding against an old deployment that lacks these APIs.
...
- `api.mcpActivity.get({})`: `lastSuccessfulReadAt`, `lastSuccessfulWriteAt`, and `testMarker`, all nullable.
- `api.mcpActivity.clearConnectionTest({})`: removes the user-owned marker. The UI waits for the mutation/query result; it does not optimistically remove it.
- MCP onboarding prompt: `list_projects`, followed by `test_connection`. It requests no paid research. `remove_connection_test` is also explained. Copying instructions never marks access successful.

The onboarding page resumes from server activity on refresh and is available from workspace navigation. Its success wording deliberately describes historical observed operations, not a guarantee that a connector is still online. An empty project list is a valid read result.
...
- `components/projects/agent-onboarding.tsx`, `app/(app)/onboarding/*`: connection instructions, observed activity, reversible marker removal, recovery UI.
...
Outstanding smoke tests remain a real authenticated browser/OAuth connection (including observed read, test marker, and cleanup) and live Exa execution. Neither test was simulated as a successful integration, and no paid call was made by the UI work.
